...
### ✅ pgvector semantic search (March 11, 2026)
- **Was:** `store/embedding-repository.ts` had vector search stubbed with application-side cosine similarity. Semantic search was functional but slow (fetched 500 candidates, computed similarity in Node.js).
- **Now:** Real pgvector cosine similarity search wired using database-side `<=>` operator. Semantic search now uses PostgreSQL's vector index for fast similarity queries.
- **What was done:**
  - Replaced application-side cosine similarity with pgvector SQL query using `<=>` operator
  - Implemented proper parameterization: `${vectorString}::vector` cast for pgvector compatibility
  - Added similarity threshold (0.3) to filter out low-quality matches
  - Added graceful fallback to application-side search if pgvector extension is not available
  - Query structure: `SELECT ... 1 - (embedding <=> vector) as similarity FROM context_embeddings JOIN context_items WHERE workspaceId = ? ORDER BY embedding <=> vector LIMIT ?`
  - Maintained workspace scoping and optional type filtering
  - TypeScript passes clean (`npm run typecheck`)
- **Impact:** Loopbrain semantic search now scales efficiently. Query time reduced from O(n) application-side computation to O(log n) database-side vector index lookup.
- **Note:** Embeddings are generated by `embedding-service.ts` using OpenAI text-embedding-3-small (1536 dimensions). Indexing pipeline (`indexing/indexer.ts`) populates embeddings for pages, projects, tasks, people, teams, roles, etc.
