...
### pgvector: Float[] column, JS fallback active
- **What:** `ContextEmbedding.embedding` is `Float[]` in the Prisma schema, not the pgvector `Vector` type. The `<=>` cosine distance operator in `searchEmbeddings()` fails at runtime with `operator does not exist`. The code catches this and falls back to application-side cosine similarity over up to 500 candidates fetched into Node.js memory.
- **Why it matters:** JS-side search is O(n) per query — fine at beta scale (Acme Analytics: tens of embeddings). Will degrade noticeably past a few thousand ContextItems. Also means no HNSW index benefit.
- **Fix (half-day, post-beta):**
  1. Enable pgvector extension in Supabase: `CREATE EXTENSION IF NOT EXISTS vector;`
  2. Prisma migration: change `embedding Float[]` → `embedding Unsupported("vector(1536)")`
  3. Add HNSW index: `@@index([embedding], type: Hnsw, opclass: VectorCosineOps)`
  4. Wire `backfillWorkspaceEmbeddings()` as a one-off admin route to populate historical ContextItems (the backfill helper exists in `src/lib/loopbrain/embedding-backfill.ts` but has no API route)
- **Current behavior:** `<=>` operator fails → caught → falls back to JS cosine similarity. Transparent to users. `search_wiki` tool works, just slowly at scale.
- **Effort:** ~4 hours. Blocked on: Supabase pgvector extension being enabled.
- **Risk:** Migration requires downtime window or zero-downtime column swap. Test on branch DB first.
- **Files:** `prisma/schema.prisma` (ContextEmbedding model), `src/lib/loopbrain/store/embedding-repository.ts`, `src/lib/loopbrain/embedding-backfill.ts`

### Proactive insights cron not scheduled
- **What:** `/api/cron/insights` exists and is fully implemented but is **not listed in `vercel.json`**. Insights are never auto-generated. The only way to trigger generation is a manual `POST /api/loopbrain/insights` (requires ADMIN role).
- **Why it matters:** Dashboard `insights-card.tsx` shows `GET /api/loopbrain/insights`. If the cron never runs, the card is always empty. The feature appears broken to users even though the pipeline works.
- **Fix:** Add to `vercel.json` crons array: `{ "path": "/api/cron/insights", "schedule": "*/15 * * * *" }` (every 15 minutes — the route's own `recommendedSchedule`). Also set `LOOPBRAIN_CRON_SECRET` env var on Vercel.
- **Effort:** 5 minutes + deploy.
- **Risk:** Low. The cron processes all active workspaces, max 100, 5-minute timeout. Watch first run logs to verify Acme workspace produces insights.
- **Pre-demo action:** Manually trigger via `POST /api/loopbrain/insights` as an ADMIN to populate the Acme workspace before the demo. Then add the cron entry.
- **Files:** `vercel.json`, `src/app/api/cron/insights/route.ts`
...
### ~~pgvector semantic search~~ — REVERTED TO P1 (March 15, 2026)
- **Marked done prematurely.** See P1 item "pgvector: Float[] column, JS fallback active" below.
