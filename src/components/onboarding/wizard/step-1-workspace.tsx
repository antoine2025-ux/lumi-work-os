...
export function Step1Workspace({ defaultValues, submitting, onSubmit }: Step1WorkspaceProps) {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (!canSubmit) {
      ...
      } else if (adminName.trim().length < 1) {
        setValidationError('Please enter your name')
      } else if (adminTitle.trim().length < 1) {
        setValidationError('Please enter your role or title')
      } else if (companySize === '') {
  ...
  return (
    <Card className="border-border/50" data-testid="onboarding-step">
      ...
      <CardContent className="space-y-5 pt-4">
        ...
        >
          ...
          {/* Submit - always clickable; validation runs on submit */}
          <Button
            type="submit"
