...
export default async function OrgDepartmentDetailPage({ params }: PageProps) {
  const department = await getOrgDepartmentById(params.id);

  if (!department) {
    return notFound();
  }

  const { id, name, description, teamCount, isActive } = department;
  ...
  return (
    <div className="p-8 space-y-6">
      <OrgPageHeader
        title={name}
        subtitle="Department details and teams overview."
        breadcrumbs={[
          { label: "Org", href: "/org" },
      ...
      <section className="space-y-3 rounded-md border bg-card p-4 text-sm">
        <div className="flex items-center justify-between gap-2">
          ...
          <Link
            href={`/org/teams?departmentId=${encodeURIComponent(id)}`}
      ...
      <DepartmentLoopbrainPanelClient departmentId={id} departmentName={name} />
