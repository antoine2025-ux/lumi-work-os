...
/**
 * Reusable Org activity screen.
 * Contains the existing activity feed UI from L7/L8.
 * Can be rendered in the legacy dashboard route and inside the new Org Center.
 */
export async function OrgActivityScreen({ 
  orgId, 
  searchParams 
}: OrgActivityScreenProps) {
 ...
 * Keeps the old /org/[orgId]/activity route working by passing params.orgId
 * into OrgActivityScreen.
 */
export default async function Page({ 
  ...
  return <OrgActivityScreen orgId={resolvedParams.orgId} searchParams={searchParams} />;
}
